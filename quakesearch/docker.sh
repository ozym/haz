...
PKGNAME=quakesearch-go
DOCKER_TMP=docker-build-tmp

# Prefix for the logs
BUILD='-X github.com/GeoNet/quakesearch-go/vendor/github.com/GeoNet/log/logentries.Prefix=git-'`git rev-parse --short HEAD`
...
# App specific assets.
cp -Ra css/ index.html js ${DOCKER_TMP}/common

docker build -t quay.io/geonet/geonet-web:quakesearch_go .
echo Run container with cmd: docker run --name quakesearch_go -p 8080:8080 --env-file env.list -d quakesearch_go
