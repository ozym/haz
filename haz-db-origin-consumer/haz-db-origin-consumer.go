// haz-db-origin-consumer listens to an AWS SQS queue for Haz JSON messages and
// saves the messages into the DB for the origin web servers.  The db is from the geonet
// project (the qrt schema).
package main
...
func (m *message) Process() bool {
	...
	case m.HeartBeat != nil:
		m.HeartBeat.RxLog()
		m.SetErr(db.SaveHeartBeatQRT(*m.HeartBeat))
	case m.Quake != nil:
		m.Quake.RxLog()
		m.SetErr(db.SaveQuakeQRT(*m.Quake))
	}
